...
static char vcid[] = "$Id: soil_conduction.c,v 5.4.2.12 2009/12/12 19:45:20 vicadmin Exp $";
...
/**********************************************************************
  ...
  double organic       soil organic matter content (fraction)
**********************************************************************/
  double Ke;
  double Ki = 2.2;	/* thermal conductivity of ice (W/mK) */
  double Kw = 0.57;	/* thermal conductivity of water (W/mK) */
  double Ks_org = 0.25; /* thermal conductivity of organic solid (W/mK) (Farouki 1981) */
  double Ksat;
  double Ks;		/* thermal conductivity of soil (W/mK)
			   including mineral and organic components */
  double Ks_min;        /* thermal conductivity of solid (W/mK)
			   function of quartz content */
  double Kdry;          /* Dry thermal conductivity of soil, including mineral and organic fractions */
  double Kdry_org = 0.05; /* Dry thermal conductivity of organic fraction (Farouki 1981) */
  double Kdry_min;        /* Dry thermal conductivity of mineral soil */
  ...
  if(moist>0.) {
    ...
    if(quartz < .2) 
      Ks_min = pow(7.7,quartz) * pow(2.2,1.0-quartz);  // when quartz is less than 0.2
    else
      Ks_min = pow(7.7, quartz) * pow(3.0, 1.0-quartz);  // when quartz is greater than 0.2
...
/**********************************************************************
  ...
  double organic      fraction of soil volume composed of organic matter (fract)
**********************************************************************/

  double Cs;

  Cs = 2.0e6 * (soil_fract - organic);
  Cs += 4.2e6 * water_fract;
  Cs += 1.9e6 * ice_fract;
  Cs += 2.7e6 * organic;
  Cs += 1.3e3 * (1. - (soil_fract + water_fract + ice_fract + organic));
...
*********************************************************************/
  ...
  for(nidx=0;nidx<Nnodes;nidx++) {
	...
	= soil_conductivity(moist_node[nidx], moist_node[nidx] 
			    - ice_node[nidx], 
			    soil_density[lidx], bulk_density[lidx], quartz[lidx], organic[lidx]);
...
/********************************************************************
  ...
  double             soil_density   soil layer soil density (kg/m^3)
  double             quartz         soil layer quartz content (fract)
  double             organic        soil layer organic content (fract) 
