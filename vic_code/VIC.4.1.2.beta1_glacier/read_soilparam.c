...
/**********************************************************************
  ...
  double organic;                fract   organic matter content of the soil
  ...
  if ( RUN_MODEL ) {
   ...
   token = strtok (NULL, delimiters);
    ...
    for(layer = 0; layer < options.Nlayer; layer++) {
      ...
      while (token != NULL && (length=strlen(token))==0) token = strtok (NULL, delimiters);
      if( token == NULL ) {
        sprintf(ErrStr,"ERROR: Can't find values for ORGANIC MATTER for layer %d in soil file\n", layer );
        nrerror(ErrStr);
      }  
      sscanf(token, "%lf", &temp.organic[layer]);
#if !OUTPUT_FORCE
      if(options.FULL_ENERGY 
	 && (temp.organic[layer] > 1. || temp.organic[layer] < 0)) {
	fprintf(stderr,"Need valid organic content as a fraction to run");
	fprintf(stderr," Full Energy model, %f is not acceptable.\n",
		temp.organic[layer]);
	exit(0);
      }
    ...
    *************************************************/
    ...
    // temp.BandIceMove   = (double *)calloc(Nbands,sizeof(double));
    temp.BandSlope     = (double *)calloc(Nbands,sizeof(double));
    // temp.BandIceDepth  = (double *)calloc(Nbands,sizeof(double));
    ...
    for (band = 0; band < Nbands; band++) {
      ...
      // temp.BandIceMove[band]  = 0.;
      temp.BandSlope[band]    = 0.;
      // temp.BandIceDepth[band]    = 0.;
      temp.Tfactor[band]   = 0.;
      temp.Pfactor[band]   = 1.;
    }
    temp.AreaFract[0] = 1.;
