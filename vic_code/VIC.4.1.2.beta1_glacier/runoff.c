...
static char vcid[] = "$Id: runoff.c,v 5.7.2.20 2010/02/08 02:30:01 vicadmin Exp $";
...
{  
  ...
  /* Add variables for water table calculation. */
  double             total_depth, wetness;
...
//	    }
	  ...
	  for (time_step = 0; time_step < dt; time_step++) {
	    ...
	    **************************************************/
		...
		liq[lindex] = resid_moist[lindex] - ice[lindex];
 	    ...
 	    /***************************************************
             Compute water table depth (calculation similar to that used in DHSVM).
 	    **************************************************/

	    /** Update moisture content of the bottom soil layer **/ 
	    
	    total_depth = 0.0;
	    /* Find total soil depth. */
	     for(lindex = options.Nlayer - 1; lindex>=0; lindex--)
	       total_depth += soil_con->depth[lindex];
	    
	    dt_wtable = 0.; /* land class average water table */
	    lindex = options.Nlayer - 1;

 	    /* First find saturated thickness in bottom soil layer. */
	    if((liq[lindex] + ice[lindex]) >=  soil_con->max_moist[lindex]) {
 	      dt_wtable  =  soil_con->depth[lindex];
	      
	      /* Check to see if water table extends into upper soil layers. */
	      for(lindex = options.Nlayer - 2; lindex>=0; lindex--)
		{
		  /* Saturated thickness in this layer */
		  wetness = ((liq[lindex] + ice[lindex])- soil_con->Wcr[lindex]) /
		    (soil_con->max_moist[lindex]-soil_con->Wcr[lindex]);
		  if( wetness < 0.)
		    wetness = 0.;
 
		  dt_wtable  +=  soil_con->depth[lindex]*wetness;
		}
	    }
 	    else {  /* Bottom layer is not completely saturated. */	      
 	      lindex = options.Nlayer - 1;
	      
	      if( (liq[lindex] + ice[lindex])  >=  soil_con->Wcr[lindex]) 
		{
		  wetness = ((liq[lindex] + ice[lindex])- soil_con->Wcr[lindex]) /
		    (soil_con->max_moist[lindex]-soil_con->Wcr[lindex]);
		  dt_wtable  =  soil_con->depth[lindex]*wetness;		  
	    ...
	    /* Convert saturated thickness to depth to water table from surface */
	 
	    dt_wtable = total_depth - dt_wtable;
	...
	cell->wtable   += wtable[frost_area] * frost_fract[frost_area];
#else
	layer[lindex].moist = liq[lindex] + ice[lindex];      
        cell->runoff   += runoff[frost_area];
        cell->baseflow += baseflow[frost_area];
	cell->wtable   += wtable[frost_area];
#endif // SPATIAL_FROST
  ...
  if(options.FULL_ENERGY || options.FROZEN_SOIL) {
    ...
    if(SubsidenceUpdate == 0 ){
      ...
      ErrorFlag = distribute_node_moisture_properties(energy->moist, energy->ice,
						      ...
						      soil_con->quartz, soil_con->organic, Nnodes, 
