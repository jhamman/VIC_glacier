...
**********************************************************************/
{
  ...
  if ( Nbands > 1 ) {
    ...
    //	      total);
    // for ( band = 0; band < options.SNOW_BAND; band++ ) 
    //	soil_con->GlAreaFract[band] /= total;
    // }
    /** Read Band Elevation **/
    avg_elev = 0;
    for ( band = 0; band < Nbands; band++ ) {
    ...
    for ( band = 0; band < Nbands; band++ ) {
      ...
      if ( band_slope < 0 ) {
	fprintf(stderr,"Negative surface slope (%f) read from file\n", 
		band_slope);
      }
     ...
     /** Read Band initial Ice thicknes in glacier cells **/
    avg_elev = 0;
    for ( band = 0; band < Nbands; band++ ) {
      fscanf(snowband, "%lf", &band_thick);
      if ( band_thick < 0 ) {
	fprintf(stderr,"Negative Ice thickeness in snow band (%f) read from file\n", 
		band_thick);
      }
