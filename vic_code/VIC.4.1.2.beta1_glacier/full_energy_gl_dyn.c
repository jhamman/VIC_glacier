...
static char vcid[] = "$Id: full_energy.c,v 5.8.2.22 2010/04/02 23:20:07 vicadmin Exp $";
...
**********************************************************************/
{
 ...
 /** Bibi: Glacier flow routine **/
	  // if (options.GLACIER = TRUE) {
	  // gl_flow (
	  // }
	 
	  if( soil_con->glcel == 1){

	    // && snow[iveg][band-1].iwq > 0 && snow[iveg][band].iwq > 0 && band > 0) {
	   
	    Initialiwqup = snow[iveg][band-1].iwq;
	   Initialiwqlow = snow[iveg][band].iwq;
	   radangle = PI * soil_con->BandSlope[band-1]/ 180.0;
	   tau = 0.646 * 917. * 9.81 * snow[iveg][band-1].iwq * sin(radangle);
	        
	 
	   ud = 0.4 * (double)6.8E-23 * snow[iveg][band-1].iwq * pow((double)tau,3);
	        
	  
	   us = 3.1709E-6 * exp(1 - (100000/tau)); //Uc = 100m/year calibrated
	   
	   snow[iveg][band-1].icevel = (ud + us) * 10800.; 
	
	   snow[iveg][band-1].icemove = (snow[iveg][band-1].icedepth * (2 * snow[iveg][band-1].icedepth)) * snow[iveg][band-1].icevel;  /* Volume of ice movement in cubic meters/time step. The glacier width is assumed to be 2000 meters. */
	   
	   //printf("icemove band (m/timestep) %.3f %.3f %.3f %.12f\n",snow[iveg][band-1].icemove,snow[iveg][band].iwq,us,snow[iveg][band-1].icevel);
	   
	   BandIceDepthBL = snow[iveg][band-1].icemove/(soil_con->AreaFract[band]*soil_con->cell_area);
	   
	   IWQBH = 0;
	   IWQBH = BandIceDepthBL * (880.0 / 1000.);
	   snow[iveg][band].iwq =  snow[iveg][band].iwq +  IWQBH ;
	
	   BandIceDepthBH = snow[iveg][band-1].icemove/(soil_con->AreaFract[band-1] * soil_con->cell_area);      // IceThickBH;
	   IWQBH = BandIceDepthBH * (880.0 / 1000.);
	   snow[iveg][band-1].iwq =  snow[iveg][band-1].iwq -  IWQBH ;
	   /* Mass balance test */
	   //MassBalanceError = (Initialiwqup - snow[iveg][band-1].iwq) + (Initialiwqlow - snow[iveg][band].iwq); 
	   //	if(rec==105)
	   //printf(" %f %f %f %f %g\n",Initialiwqup, snow[iveg][band-1].iwq, Initialiwqlow, snow[iveg][band].iwq, MassBalanceError * 1000);
	 }
